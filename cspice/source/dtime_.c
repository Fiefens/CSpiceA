...
/*
 * dtime_ replacement for Windows and portable C compilers.
 * Returns elapsed wall-clock time in seconds since last call.
 * Sets tarray[0] = elapsed user time
 * Sets tarray[1] = 0 (no system time available)
 */

double dtime_(real* tarray)
{
    static clock_t last_clock = 0;
    clock_t now = clock();

    double elapsed = (double)(now - last_clock) / CLOCKS_PER_SEC;
    tarray[0] = (real)elapsed;
    tarray[1] = 0.0f;
